...
export const ProductCardsContainer = ({ className }: IProductCardsContainerProps) => {
  const { data : productsData } = useGetAllProductsQuery(undefined);

  const productCardsData: TProductCards = productsData?.data?.map((item: IProduct) => ({
    name: item.title,
    image: {
      src: item.thumbnail,
...
const ProductCard = ({
  ...
  return (
    <div className="relative group ">
      <Card className="min-w-[300px] min-h-[400px] overflow-hidden p-0 rounded-tl-none rounded-tr-3xl rounded-bl-3xl rounded-br-3xl">
